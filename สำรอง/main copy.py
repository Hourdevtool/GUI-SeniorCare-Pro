...
class Frame3(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        back_button = ctk.CTkButton(navbar, text="←", width=150, height=100, corner_radius=35,
                                    fg_color="#2D6A4F", hover_color="#FF6B6B", text_color="white",
...
class info(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        back_button = ctk.CTkButton(navbar, text="←", width=120, height=80, corner_radius=35,
                                    fg_color="#2D6A4F", hover_color="#FF0000", text_color="white",
                                    font=("Arial", 40, "bold"), command=go_back)
...
class Report1(ctk.CTkFrame):
    ...
    def display_table(self):
        ...
        if not self.data:
            no_data_label = ctk.CTkLabel(self.table_frame, text="ไม่มีประวัติการจ่ายยา", font=("TH Sarabun New", 18, "bold"))
            no_data_label.grid(row=0, column=0, columnspan=3, padx=15, pady=8)
            return  # Exit the method to prevent the rest of the code from running if there's no data
...
class Report2(ctk.CTkFrame):    
    ...
    def __init__(self, parent, controller):
        ...
        # Navbar
        navbar = ctk.CTkFrame(self, height=100, fg_color="#ffffff", corner_radius=0)
        navbar.pack(side="bottom", fill="x")
        page_title = ctk.CTkLabel(navbar, text="หน้าพิมพ์รายงาน", font=("Arial", 50, "bold"), text_color="black")
        page_title.pack(side="left", padx=20)
        back_button = ctk.CTkButton(navbar, text="←", width=150, height=100, corner_radius=35, 
                                    fg_color="#007ACC", hover_color="#005999", text_color="white",
                                    font=("Arial", 44, "bold"),
                                    command=lambda: controller.show_frame(HomePage))
        back_button.pack(side="right", padx=10, pady=5)

        # Scrollable Frame for the table
        self.scroll_frame = ctk.CTkScrollableFrame(self, width=1800, height=800, fg_color="transparent")
        self.scroll_frame.place(x=60, y=100)
    ...
    def display_data(self, data, advices):
        ...
        for col, header in enumerate(self.headers):
            label = ctk.CTkLabel(self.scroll_frame, text=header, font=("Arial", 20, "bold"), text_color="white", width=self.column_widths[col])
